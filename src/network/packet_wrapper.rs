...
impl<'a> PacketWrapper<'a> {
    pub fn new(packet: &'a EthernetPacket<'a>) -> PacketWrapper<'a> {
    ...
    pub fn get_ip_version(&self) -> Option<u8> {
        match self {
    ...
    pub fn get_header_protocol(&self) -> Option<String> {
        match self {
    ...
    pub fn get_sub_protocol(&self, port: u16) -> Option<String> {
        ProtocolPort::from(port).to_string().into()
    }
}
